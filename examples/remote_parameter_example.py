...
sys.path.append(CURRENT_DIRECTORY[:CURRENT_DIRECTORY.rindex('/')] + '/builddir/')
...
def remote_listener(parameter_identifier: int | str):
    ...
    import pycsh
    pycsh.init(yamlname=f"{CURRENT_DIRECTORY}/remote.yaml", stdout=DEVNULL)
    ...
    remote_print("Let's wrap up by changing it back to its original value.")
...
def local_sender(parameter_identifier: int | str):
    ...
    import pycsh
    pycsh.init(yamlname=f"{CURRENT_DIRECTORY}/local.yaml", stdout=DEVNULL)
